...
app.patch('/api/teacher/exams/:id', async(req,res)=>{
  try{
    ...
    await pool.query('UPDATE exams SET title=$1,type=$2,pdf_data_url=$3,questions_json=$4,student_password=$5,duration_ms=$6,folder_id=$7,allow_retake=$8,max_attempts=$9 WHERE id=$10 AND owner_user_id=$11',[title,type,pdf,type==='template'?JSON.stringify(questions):null,password,duration,folderId,allowRetake,maxAttempts,e.id,u.id]);
...
async function showExam(d){examData=d;$('portal').style.display='none';$('app').style.display='block';$('examTitle').textContent=d.title||EXAM_TITLE;$('pdfSubmitBtn').classList.toggle('hidden',d.type!=='pdf');if(d.type==='pdf')$('pdfSubmitBtn').onclick=()=>submitExam(false);if(d.type === 'template')renderTemplate();else await renderPDF(d.pdfDataUrl);startTimer()}
...
function submitPdfOnClose(){if(examData?.type!=='pdf'||!session||session.finishedAt||!studentAuthToken)return;const payload=JSON.stringify({sessionToken:session.sessionToken,answers:{}});try{fetch(API+'/finish',{method:'POST',keepalive:true,headers:{'Content-Type':'application/json','Authorization':'Bearer '+studentAuthToken},body:payload});}catch(_){} }
