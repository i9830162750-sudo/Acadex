...
function makeDeviceId(){ return crypto.randomBytes(24).toString('hex'); }
  ...
  try{
    ...
    if(ds){
      ...
      if(ds.student_id===studentId){
        ...
        return res.json({sessionToken:ds.token,studentId:ds.student_id,studentName:ds.student_name,deviceId:ds.device_id,startedAt:Number(ds.started_at),endAt:Number(ds.end_at),...publicExam(exam)});
      }
...
<script>
const EXAM_ID=${safeId}; const EXAM_TITLE=${safeTitle};
const API='/api/exam/'+encodeURIComponent(EXAM_ID); const DB_NAME='exam-tool-student'; const STORE='sessions';
...
async function save(v){const db=await idb();return new Promise((resolve,reject)=>{const r=db.transaction(STORE,'readwrite').objectStore(STORE).put(v);r.onsuccess=resolve;r.onerror=()=>reject(r.error)})}
